...
MAX_OUTPUT_VOLT = 1.5;               % 期望最大输出电压
MIN_OUTPUT_VOLT = -1.5;              % 期望最小输出电压
AMPLITUDE_VOLT = (MAX_OUTPUT_VOLT - MIN_OUTPUT_VOLT) / 2; % 幅度 = 1.5V
DC_OFFSET_VOLT = 0;                  % 直流偏移 = 0V (以0V为中心)

% 计算对应的数字值
ADC = ZERO_VOLT_CODE + round(DC_OFFSET_VOLT / LSB_VOLTAGE);  % 直流分量 (对应0V)
...
plot(s);                %绘制图形
%创建coe文件
fild = fopen('sin_wave_256x14_neg1p5_to_pos1p5.coe','wt');  % -1.5V~+1.5V正弦波文件
%写入coe文件头
